...
def build_opening_book(agent1, agent2, time_limit=16,size=5):
    """
    Builds the opening book for MCTS. Only runs get_move once for MCTS, does not simulate an entire game
    ...
    """
    # if agent1 is MCTS, set opening book params
    if isinstance(agent1, MCTSAgent):
        agent1.build_opening_book = True 
        agent1.opening_book_depth = 1
    else: # if agent2 is MCTS, set opening book params
        agent2.build_opening_book = True
        agent2.opening_book_depth = 2

    my_go = GoProblem(size=size)
    state = my_go.start_state

    # if MCTS starts, call get_move on an empty board
    if isinstance(agent1, MCTSAgent):
        state = my_go.start_state
        agent1.get_move(state.clone(), time_limit) # build tree
    else: # if MCTS goes second, call get_move for every possible first move
        for opp_move in range(size**2-1):
            # first player plays opp_move, second player is mcts. Not simulating entire game
            player1_action = opp_move
            state = my_go.transition(state, player1_action)
            agent2.get_move(state.clone(), time_limit) #build tree
...
def create_agent(agent_type: str, **kwargs):
    ...
    if agent_type.lower() == "alphabeta":
       ...
       model = load_model(model_path, ValueNetwork(80))
       heuristic_search_problem = GoProblemLearnedHeuristic(model)
       return IterativeDeepeningAgent(1)
    elif agent_type.lower() == "final":
        return FinalAgent(kwargs.get("size"))
    elif agent_type.lower() == "mctsoriginal":
        return MCTSOriginalAgent()
...
def parse_args():
    ...
    # added build opening book argument
    parser.add_argument('--build-book', choices=["2",'0',"1"],default="2",
                      help='Which starting move the MCTS agent plays when building the opening book')
...
def main():
    ...
    # if build book with MCTS as player 0
    if args.build_book == "0":
        build_opening_book(MCTSAgent(game_size=args.size),None,size=args.size)
        return
    elif args.build_book == "1": # if build book with MCTS as player 1
        build_opening_book(None,MCTSAgent(game_size=args.size),size=args.size)
        return
